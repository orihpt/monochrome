...
import { initProfileEditPage } from './profile.js';
...
export class UIRenderer {
    ...
    setSectionVisible(element, visible) {
    ...
    initializeHeaderAccount() {
        authManager.onAuthStateChanged(async (user) => {
            await this.updateHeaderAccountIcon(user);
        });
    }

    async updateHeaderAccountIcon(user) {
        const iconWrap = document.getElementById('header-account-icon');
        const img = document.getElementById('header-account-img');
        const btn = document.getElementById('header-account-btn');

        if (!iconWrap || !img || !btn) return;

        let avatarUrl = '/assets/user.png';
        let displayName = 'Account';

        const subsonicUser = localStorage.getItem('subsonic_user');
        if (subsonicUser) {
            displayName = subsonicUser;
            try {
                const fullUser = await this.api.subsonicAPI.getCurrentUser();
                if (fullUser) {
                    this.user = {
                        ...fullUser,
                        isAdmin: fullUser.adminRole === true || fullUser.adminRole === 'true'
                    };
                    displayName = fullUser.name || fullUser.username;
                    if (fullUser.id) {
                        avatarUrl = `/api/user/${encodeURIComponent(fullUser.id)}/avatar`;
                    }
                }
            } catch (e) {
                console.warn('Failed to fetch full user for header icon', e);
            }
        } else if (user) {
        ...
        img.onerror = () => {
            ...
            const defaultIcon = document.getElementById('header-account-default-icon');
        ...
        const dropdownName = document.getElementById('header-account-display-name');
    ...
    async updateRecommendationAvailability() {
        ...
        list.innerHTML = (
            await Promise.all(pinnedItems.map(async (item) => {
                ...
                if (item.type === 'user-playlist' && coverMetadata.coverType === PLAYLIST_COVER_TYPE_STYLISH) {
                    ...
                    iconHTML = `<img src="${coverUrl}" class="pinned-item-cover ${coverClass}" alt="${escapeHtml(item.name)}" loading="lazy" onerror="this.src='/assets/no_album_cover.png'">`;
                }
    ...
    createTrackItemHTML(
        ...
        if (track.isLocal && (!track.album?.cover || track.album.cover === '/assets/no_album_cover.png')) {
            showCover = false;
        }
    ...
    getCoverHTML(
        ...
        const fallbackUrl = type === 'artist' ? this.api.getCoverUrl(artistFallbackId, size) : '/assets/no_album_cover.png';
        const finalFallback = type === 'artist' ? '/assets/artist_placeholder.png' : '/assets/no_album_cover.png';
        return `<img src="${imageUrl}" class="${className}" alt="${alt}" loading="${loading}" onerror="this.src='${fallbackUrl}'; this.onerror=function(){this.src='${finalFallback}'; this.onerror=null;};">`;
    }

    getPlaylistCoverHTML(playlist, className = 'card-image', loading = 'lazy') {
        ...
        if (coverMetadata.coverType === PLAYLIST_COVER_TYPE_STYLISH) {
            return createStylishCoverHTML(
                { ...playlist, name: name },
                coverMetadata,
                `${className} playlist-cover-rendered playlist-cover-preview`
            );
        }

        if (coverMetadata.coverType === PLAYLIST_COVER_TYPE_UPLOADED && coverMetadata.uploadedCoverId) {
            const coverId = coverMetadata.uploadedCoverId;
            if (isIndexedDbCoverId(coverId)) {
                queueMicrotask(() => this.hydratePlaylistUploadedCovers());
                return `<img src="/assets/no_album_cover.png" data-playlist-uploaded-cover-id="${escapeHtml(coverId)}" alt="${safeName}" class="${className} playlist-cover-rendered playlist-cover-uploaded" loading="${loading}">`;
            } else {
                // If it looks like a Navidrome ID (not a URL and not a blob), wrap it in getCoverUrl
                const src = (coverId.startsWith('http') || coverId.startsWith('blob:') || coverId.startsWith('/'))
                    ? coverId
                    : this.api.getCoverUrl(coverId);
                return `<img src="${escapeHtml(src)}" alt="${safeName}" class="${className} playlist-cover-rendered playlist-cover-uploaded" loading="${loading}" onerror="this.src='/assets/no_album_cover.png'">`;
            }
        }

        const albumCovers = getPlaylistAlbumCovers(playlist);
        if (coverMetadata.coverType === PLAYLIST_COVER_TYPE_ALBUM_GRID && albumCovers.length > 0) {
            return `<div class="${className} playlist-cover-rendered playlist-cover-preview">${createAlbumGridHTML(playlist, this.api, loading)}</div>`;
        }

        // Fallback for standard playlists or empty grids: use the Navidrome-generated collage/image
        const imageId = playlist.squareImage || playlist.image || playlist.cover || playlist.uuid;
        if (imageId && typeof imageId === 'string' && !imageId.startsWith('placeholder:')) {
            return `<img src="${this.api.getCoverUrl(imageId)}" alt="${safeName}" class="${className}" loading="${loading}" onerror="this.src='/assets/no_album_cover.png'">`;
        }

        // Final fallback: empty grid (placeholders)
        return `<div class="${className} playlist-cover-rendered playlist-cover-preview">${createAlbumGridHTML(playlist, this.api, loading)}</div>`;
    }
    ...
    createPlaylistCardHTML(playlist) {
        ...
        const imageHTML = this.getPlaylistCoverHTML(playlist, 'card-image', 'lazy');
    ...
    createMixCardHTML(mix) {
        ...
        return this.createBaseCardHTML({
            ...
            imageHTML: this.getPlaylistCoverHTML(mix, 'card-image', 'lazy'),
            actionButtonsHTML: `
    ...
    createUserCardHTML(user) {
        const avatar = user.avatarFile ? `/api/user/${encodeURIComponent(user.id)}/avatar` : '/assets/user.png';
        const name = user.display_name || user.userName || user.name || 'User';
        const isCompact = cardSettings.isCompactAlbum();
        ...
        return this.createBaseCardHTML({
            ...
            imageHTML: `
                    <img src="${avatar}" alt="${escapeHtml(name)}" class="card-image" loading="lazy" onerror="this.src='/assets/user.png'">
            `,
            actionButtonsHTML: '',
            isCompact,
        });
    ...
    createUserPlaylistCardHTML(playlist, customSubtitle = null) {
        const loading = playlist._lazy === false ? 'eager' : 'lazy';
        const imageHTML = this.getPlaylistCoverHTML(playlist, 'card-image', loading);
        ...
        return this.createBaseCardHTML({
            ...
            imageHTML: imageHTML,
            actionButtonsHTML: `
                <button class="edit-playlist-btn" data-action="edit-playlist" title="עריכת הפלייליסט">
                    ${SVG_SQUARE_PEN(20)}
                </button>
    ...
    createAlbumCardHTML(album) {
        ...
        return this.createBaseCardHTML({
            ...
            extraAttributes: `data-testid="album-card"${isBlocked
    ...
    async renderLibraryPage() {
        ...
        if (myPlaylistsContainer) {
            ...
            myPlaylistsContainer.querySelectorAll('.liked-songs-card').forEach((el) => el.remove());
            myPlaylistsContainer.querySelectorAll('.placeholder-text').forEach((el) => el.remove());

            if (likedTracks.length) {
                myPlaylistsContainer.insertAdjacentHTML('beforeend', this.createLikedSongsCardHTML(likedTracks.length));
                const likedCard = myPlaylistsContainer.querySelector('[data-user-playlist-id="liked-songs"]');
                if (likedCard) {
                    trackDataStore.set(likedCard, {
                        id: 'liked-songs',
                        uuid: 'liked-songs',
                        title: 'שירים שאהבתם',
                        name: 'שירים שאהבתם',
    ...
    async renderHomePage() {
        ...
        try {
            ...
            await this.updateRecommendationAvailability();
    ...
    async setupHomeTabs() {
        ...
        if (showAllMixesBtn && !showAllMixesBtn.dataset.initialized) {
            ...
            showAllMixesBtn.addEventListener('click', async () => {
                const tab = document.querySelector('.home-tab[data-tab="curator-mixes"]') || {
                    click: () => {
        ...
        for (const tab of tabs) {
            ...
            tab.addEventListener('click', async () => {
                ...
                } else if (tab.dataset.tab === 'community') {
                    await this.renderCommunityPage();
    ...
    async renderCommunityPage() {
        const recentlyEl = document.getElementById('community-recently-played');
        const mostEl = document.getElementById('community-most-played');
        const followingEl = document.getElementById('community-following-played');
        const featuredEl = document.getElementById('community-featured-playlists');
        if (!recentlyEl || !mostEl || !followingEl || !featuredEl) return;

        recentlyEl.innerHTML = this.createSkeletonTracks(6, true);
        mostEl.innerHTML = this.createSkeletonTracks(6, true);
        followingEl.innerHTML = this.createSkeletonTracks(6, true);
        featuredEl.innerHTML = this.createSkeletonCards(8, false);

        try {
            const [activity, featuredPlaylists] = await Promise.all([
                this.api.getCommunityActivity(20),
                this.api.getFeaturedPlaylists(24),
            ]);
            // Use this.setSectionVisible instead of local helper

            if (activity.recentlyPlayed?.length) {
                this.setSectionVisible(recentlyEl, true);
                await this.renderListWithTracks(recentlyEl, activity.recentlyPlayed, true, false, false, true);
            } else {
                recentlyEl.innerHTML = '';
                this.setSectionVisible(recentlyEl, false);
            }

            if (activity.mostPlayed?.length) {
                this.setSectionVisible(mostEl, true);
                await this.renderListWithTracks(mostEl, activity.mostPlayed, true, false, false, true);
            } else {
                mostEl.innerHTML = '';
                this.setSectionVisible(mostEl, false);
            }

            if (activity.followingRecent?.length) {
                this.setSectionVisible(followingEl, true);
                await this.renderListWithTracks(followingEl, activity.followingRecent, true, false, false, true);
            } else {
                followingEl.innerHTML = '';
                this.setSectionVisible(followingEl, false);
            }
            ...
            console.warn('Failed to load community page:', error);
            const errorMsg = createPlaceholder('לא ניתן לטעון את פעילות הקהילה.');
            recentlyEl.innerHTML = errorMsg;
            mostEl.innerHTML = errorMsg;
    ...
    async renderHomeRecentlyAddedTracks() {
        ...
        try {
            ...
            console.error('Failed to load recently added tracks:', e);
            container.innerHTML = '';
            this.setSectionVisible(container, false);
        }
    }
    ...
    async renderHomeSongs(forceRefresh = false, providedSeeds = null) {
        ...
        this.setSectionVisible(songsContainer, false);
        ...
        if (songsContainer) {
            ...
            try {
                ...
                if (filteredTracks.length > 0) {
                    this.setSectionVisible(songsContainer, true);
    ...
    async renderHomeAlbums(forceRefresh = false, providedSeeds = null, retryCount = 0) {
        ...
        this.setSectionVisible(albumsContainer, false);
        ...
        if (albumsContainer) {
            ...
            try {
                ...
                if (albumsToRender.length > 0) {
                    this.setSectionVisible(albumsContainer, true);
    ...
    createSearchAllResultHTML(kind, item, index) {
        ...
        if (isTrack) {
            ...
            title = escapeHtml(item.name || item.userName || 'User');
            subtitle = escapeHtml([typeLabel, item.userName ? `@${item.userName}` : ''].filter(Boolean).join(' • '));
            imageHTML = `<img src="${item.avatarFile ? `/api/user/${encodeURIComponent(item.id)}/avatar` : '/assets/user.png'}" alt="${title}" class="search-all-image" loading="${isFeatured ? 'eager' : 'lazy'}" onerror="this.src='/assets/user.png'">`;
            actionHTML = `
                <button class="btn-secondary search-all-action" data-action="follow-user" data-user-id="${escapeHtml(id)}" title="Follow">
                    Follow
                </button>
            `;
            extraClasses = 'user';
    ...
    createLikedSongsCardHTML(trackCount = 0) {
        return this.createBaseCardHTML({
            ...
            imageHTML: '<img src="/assets/liked_cover_512w.png" alt="שירים שאהבתם" class="card-image" loading="lazy">',
            actionButtonsHTML: '',
            isCompact: cardSettings.isCompactAlbum(),
            extraClasses: 'liked-songs-card',
        });
    }

    async renderLikedSongsPage() {
        ...
        const savedSort = localStorage.getItem('playlist-sort-liked-songs');
        let currentSort = savedSort || 'added-newest';
        let currentTracks = sortTracks(originalTracks, currentSort);

        if (collageEl) collageEl.style.display = 'none';
        imageEl.src = '/assets/liked_cover_512w.png';
        ...
        this.setPageBackground('/assets/liked_cover_512w.png');
        await this.extractAndApplyColor('/assets/liked_cover_512w.png');
        ...
        const renderTracks = async () => {
            ...
            if (currentTracks.length) {
                ...
                tracklistContainer.innerHTML += createPlaceholder('No liked tracks yet.');
            }
        };

        const applySort = async (sortType) => {
            currentSort = sortType;
            localStorage.setItem('playlist-sort-liked-songs', sortType);
        ...
        if (playBtn) {
            playBtn.innerHTML = `${SVG_PLAY(20)}<span>Play</span>`;
            playBtn.onclick = () => {
        ...
        await this.updatePlaylistHeaderActions(
            { id: 'liked-songs', uuid: 'liked-songs', title: 'שירים שאהבתם', name: 'Liked Songs' },
            false,
            currentTracks,
            false,
            applySort,
        ...
        document.title = 'שירים שאהבתם - Waves';
    ...
    async renderHomeArtists(forceRefresh = false, providedSeeds = null) {
        ...
        this.setSectionVisible(artistsContainer, false);
            ...
            try {
                ...
                if (artistId) {
                    ...
                    if (filteredArtists.length > 0) {
                        this.setSectionVisible(artistsContainer, true);
    ...
    async renderHomeRecent() {
        ...
        this.setSectionVisible(recentContainer, false);
        ...
        if (recentContainer) {
            ...
            if (displayItems.length > 0) {
                this.setSectionVisible(recentContainer, true);
    ...
    async renderSearchPage(query) {
        ...
        searchPage?.querySelectorAll('.search-tab-content').forEach((content) => {
            content.classList.toggle('active', content.id === 'search-tab-all');
        });
        ...
        if (usersContainer) usersContainer.innerHTML = this.createSkeletonCards(6, true);
        ...
        const userSearchPromise = this.api.searchUsers(query, 12).catch((error) => {
        ...
        try {
            ...
            const userItems = finalUsers.slice(0, 4).map((item, sourceIndex) => ({
                ...
                item,
                sourceIndex,
                rank: this.rankUserSearchResult(item, normalizedQuery, sourceIndex),
            }));
            const allItems = [
            ...
            if (usersContainer) {
                usersContainer.innerHTML = finalUsers.length
                    ? finalUsers.map((user) => this.createUserCardHTML(user)).join('')
                    : createPlaceholder('No users found.');
                for (const user of finalUsers) {
                    const el = usersContainer.querySelector(`[data-user-id="${CSS.escape(String(user.id))}"]`);
                    if (el) trackDataStore.set(el, user);
                }
            }
    ...
    rankUserSearchResult(user, normalizedQuery, sourceIndex = 0) {
        const names = [user.userName, user.name].filter(Boolean).map((value) => String(value).toLowerCase());
        if (names.some((name) => name === normalizedQuery)) return 120 - sourceIndex;
        if (names.some((name) => name.startsWith(normalizedQuery))) return 92 - sourceIndex;
        if (names.some((name) => name.includes(normalizedQuery))) return 70 - sourceIndex;
        return 0;
    }
    ...
    async loadRecommendedSongsForPlaylist(tracks, forceRefresh = false) {
        ...
        if (!(await this.updateRecommendationAvailability())) {
    ...
    async renderPlaylistPage(playlistId, source = null, _provider = null) {
        ...
        try {
            ...
            if (playlistData) {
                ...
                if (detailCover?.rendered) {
                    ...
                    if (uniqueCovers.length > 0 && collageEl) {
                        ...
                        imageEl.src = '/assets/no_album_cover.png';
                        imageEl.style.display = 'block';
                        if (collageEl) collageEl.style.display = 'none';
                    }
                    this.setPageBackground(null);
                ...
                if (source === 'user') {
                    throw new Error('הפלייליסט לא נמצא. אם הוא באמת קיים כדאי לוודא שהוא לא פרטי');
                }
            ...
            tracklistContainer.innerHTML = createPlaceholder(error.message);
        }
    }
    ...
    async renderArtistPage(artistId, provider = null) {
        ...
        const inLibraryLabel = document.getElementById('in-library-label');
        ...
        document.getElementById('page-artist')?.classList.add('no-artist-header');
        if (inLibraryLabel) inLibraryLabel.textContent = 'In Your Library';
                    ...
                    try {
                        ...
                        for (const track of likedTracks) {
                            if (isTrackByArtist(track)) {
                                ...
                                addSource(track.id, { label: 'שירים שאהבתם', href: '/liked-songs' });
                            }
                        }
                        ...
                        if (libraryTracks.length > 0) {
                            ...
                            if (inLibraryLabel) {
                                const likedCount = libraryTracks.length;
                                inLibraryLabel.textContent = likedCount === 1
                                    ? 'שיר אחד בספרייה'
                                    : likedCount === 2
                                        ? 'שני שירים בספרייה'
                                        : `${likedCount} שירים בספרייה`;
                            }
    ...
    async renderLyricsPage() {
        ...
        if (infoEl) this.renderLyricsPageTrackInfo(infoEl, track);
        ...
        if (track.type === 'video') {
            contentEl.innerHTML = '<div class="lyrics-error">Lyrics are unavailable for this item.</div>';
    ...
    renderLyricsPageTrackInfo(container, track) {
        if (!container) return;
        if (!track) {
            container.innerHTML = '';
            return;
        ...
        const cover = this.api.getCoverUrl(track.image || track.cover || track.album?.cover, '160');
        const albumTitle = escapeHtml(track.album?.title || '');
        const albumId = track.album?.id || '';
        container.innerHTML = `
            <img src="${cover}" alt="Current Track Cover" class="cover" loading="lazy" />
            <div class="details">
                <div class="title">${escapeHtml(getTrackTitle(track))}</div>
                <div class="album" ${albumId ? `data-album-id="${escapeHtml(albumId)}"` : ''}>${albumTitle}</div>
                <div class="artist">${getTrackArtistsHTML(track)}</div>
            </div>
        `;
    ...
    async renderProfileEditPage() {
        this.showPage('profile-edit');
        await initProfileEditPage();
    }
}
